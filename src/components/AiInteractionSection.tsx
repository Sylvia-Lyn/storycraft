...
interface AiInteractionSectionProps {
  ...
  selectModel?: (model: string) => void; // 添加selectModel函数
}
...
function AiInteractionSection({
  ...
  useEffect(() => {
    console.log(`[AiInteractionSection] selectedModel变化: ${selectedModel}`);
    if (aiSuggestions.length > 0) {
      console.log("[AiInteractionSection] 收到AI建议:", aiSuggestions);
    }
    if (inputPlaceholder) {
      console.log("[AiInteractionSection] 使用输入框占位符:", inputPlaceholder);
    }
              ...
              <div className="absolute top-full left-0 mt-1 bg-white border border-gray-200 rounded shadow-lg z-10 w-full">
                {models.map(model => (
                  <div
                    ...
                    onClick={() => {
                      console.log(`[AiInteractionSection] 点击模型选项: ${model}`);
                      if (selectModel) {
                        console.log(`[AiInteractionSection] 使用selectModel函数切换模型`);
                        selectModel(model);
                      } else {
                        console.log(`[AiInteractionSection] 使用setSelectedModel函数切换模型`);
